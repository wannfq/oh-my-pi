...
- Fixed session selector (`/resume`) not responding to the MacBook built-in delete key. The selector now accepts `Ctrl+Backspace` (modifyOtherKeys `\x1b[27;5;127~` / Kitty `\x1b[127;5u`, parsed as `ctrl+backspace`). On macOS, this acts as a forward-delete word shortcut and does not conflict with the editor's `tui.editor.deleteWordBackward` binding when the search filter has text.
